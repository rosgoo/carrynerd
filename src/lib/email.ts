...
export async function send(mail: Mail): Promise<void> {
  ...
  const res = await fetch(ENDPOINT, {
    ...
    body: JSON.stringify({
      from: process.env.ALERT_FROM ?? 'bagdex <alerts@bagdex.com>',
      to: [mail.to],
...
export function confirmEmail(confirmUrl: string, what: string): Mail {
  const text = [
    `Confirm your bagdex price watch`,
    ``,
...
<div style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;max-width:520px;color:#16181a">
  <p style="letter-spacing:.1em;text-transform:uppercase;font-size:11px;color:#8e938f">bagdex</p>
  ...
  return { to: '', subject: 'Confirm your bagdex price watch', html, text };
}
